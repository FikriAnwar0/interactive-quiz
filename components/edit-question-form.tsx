...
export function EditQuestionForm({ question, onSave }: EditQuestionFormProps) {
  ...
  useEffect(() => {
    ...
    setErrors({}) // Clear errors when question changes
  }, [question])
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave() // Close dialog and show success toast
  }
    ...
    <CardContent className="space-y-4 p-0">
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-2">
          <Label htmlFor="edit-question" className="text-base sm:text-lg">
            Pertanyaan
          </Label>
          <Input
            id="edit-question"
            ...
            aria-describedby={errors.questionText ? "edit-question-error" : undefined}
          />
          {errors.questionText && (
            <p id="edit-question-error" className="text-sm text-red-500">
              {errors.questionText}
            </p>
          )}
        </div>
        <div className="space-y-4">
          ...
          {options.map((option, index) => (
            <div key={index} className="flex items-center gap-2">
              <Input
                ...
                aria-describedby={errors[`option${index}`] ? `edit-option${index}-error` : undefined}
              />
          ...
          {errors.options && (
            <p id="edit-options-error" className="text-sm text-red-500">
              {errors.options}
            </p>
          )}
        ...
        <div className="space-y-2">
          <Label htmlFor="edit-correct-answer" className="text-base sm:text-lg">
            Jawaban Benar
          </Label>
          <Input
            ...
            aria-describedby={errors.correctAnswer ? "edit-correct-answer-error" : undefined}
          />
          {errors.correctAnswer && (
            <p id="edit-correct-answer-error" className="text-sm text-red-500">
              {errors.correctAnswer}
            </p>
          )}
        </div>
